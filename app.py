...
def database_sql(statement: str) -> str:
    """Use DB-API parameters in SQLite locally and PostgreSQL on Render."""
    return statement.replace("?", "%s") if USING_POSTGRES else statement


def db_execute(connection: Any, statement: str, parameters: tuple[Any, ...] = ()) -> Any:
    return connection.execute(database_sql(statement), parameters)


@contextmanager
def db_transaction(connection: Any) -> Iterator[None]:
    """Commit a transaction without closing Flask's request-scoped connection."""
    if USING_POSTGRES:
...
def new_connection() -> Any:
    ...
    if USING_POSTGRES:
        try:
            ...
            raise RuntimeError(
                "PostgreSQL support is unavailable. Install dependencies with "
...
def init_db() -> None:
    ...
    try:
        ...
        timestamp_column = "TIMESTAMPTZ NOT NULL" if USING_POSTGRES else "DATETIME NOT NULL"
        schema_statements = (
            """
            ...
            "CREATE INDEX IF NOT EXISTS idx_annotations_label ON annotations(assigned_label)",
            "CREATE INDEX IF NOT EXISTS idx_annotations_timestamp ON annotations(timestamp DESC)",
            """
        ...
        # does not hold a database write lock. The datasets library caches this locally.
        ...
        with db_transaction(connection):
            ...
            db_execute(connection, "DELETE FROM tweets")
            for seed_tweet in seed_tweets:
                db_execute(
                    connection,
                    "INSERT INTO tweets (id, text, true_emotion) VALUES (?, ?, ?)",
                    seed_tweet,
                )
            db_execute(
                connection,
...
def health_check():
    ...
    db_execute(get_db(), "SELECT 1").fetchone()
...
def label():
    ...
    if request.method == "POST":
        ...
        # The unique constraint makes a duplicate browser POST safe on both databases.
        with db_transaction(get_db()):
            db_execute(
                get_db(),
                """
...
def admin():
    ...
    db = get_db()
    total_labels = db_execute(db, "SELECT COUNT(*) AS count FROM annotations").fetchone()["count"]
    participant_count = db_execute(
        db,
